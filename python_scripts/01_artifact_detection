...
root_path = input("Type path to project directory: ") ## muss ich noch einrichten
...
for file in files:
    ...
    ch_ix = [raw_copy.info['ch_names'].index(chan) for chan in
             raw_copy.info['ch_names'] if chan not in ignore_ch]  # noqa <- was soll das heißen?
    ...
    if len(times) > 0:
        ...
        # Include one second before artifact onset                      # da wird dann eig schon recht viel rausgeschmissen?
